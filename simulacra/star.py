...
def download_phoenix_model(zzz, logg, alpha, temperature, outdir=None):
    temp_grid = np.concatenate(
        (np.arange(2300, 7000, 100), np.arange(7000, 12200, 200))
    )
    ...
    print(
        "your parameters: T {}, logg {}, alpha {}, z {}".format(
            temperature, logg, alpha, zzz
        )
    )
    ...
    print(
        "setting: T {}, logg {}, alpha {}, z {}".format(temperature, logg, alpha, zzz)
    )
    ...
    filename = (
        "lte{:05d}-{:1.2f}".format(temperature, logg)
        + directories[-1][1:]
...
def shifts(vel):
    return (1 / 2) * jnp.log(
        (1 + (vel / const.c).to(1.0).value) / (1 - (vel / const.c).to(1.0).value)
    )
...
def get_night_grid(loc, tstart, tend, steps_per_night=10):
    ...
    for i in range(len(sunrise) - 1):
        ...
        outarr = np.concatenate(
            (outarr, sunset[i] + np.linspace(0, nighttime.value, steps_per_night))
        )
    return outarr
...
def stellar_to_detector_flux(star, detector, exp_times):
    stellar_area = 4.0 * np.pi * star.stellar_radius**2
    ratio_of_areas = detector.area / (4.0 * np.pi * star.distance**2)

    print(
        "photon flux: {:.2e}".format(
            np.mean(
                star.wave_difference
                * star.surface_flux.to(
                    u.photon / u.s / u.m**3, u.spectral_density(star.wave)
                )
            )
            .to(u.ph / u.m**2 / u.s)
            .value
...
class PhoenixModel(TheoryModel):
    def __init__(
        ...
        self.fluxname, parameters = download_phoenix_model(
            self.z, self.logg, self.alpha, self.temperature, self.outdir
        )
    ...
    def get_velocity(self, detector, obs_times):
        rvs = get_velocity_measurements(
            obs_times, self.amplitude, self.period, detector.loc, self.target
        )
        return rvs
    ...
    def get_spectra(self, detector, obs_times):
        ...
        print(
            "surface flux: mean {:3.2e}\t median {:3.2e}".format(
    ...
    def plot(self, ax, epoch_idx, normalize=None, nargs=[]):
        ...
        if normalize is not None:
            y = normalize(y, *nargs)
        ax.plot(
            self.x - self.deltas[epoch_idx],
            y,
            "o",
            color=self.color,
            alpha=0.4,
            label="Truth " + self.__class__.__name__,
            markersize=4,
        )
        return ax
    ...
    def plot_interpolated(self, ax, epoch_idx, normalize=None, nargs=[]):
        ...
        ax.plot(
            ...
            color=self.color,
            alpha=0.3,
            label="Interpolated " + self.__class__.__name__,
            markersize=3,
        )
        return ax
