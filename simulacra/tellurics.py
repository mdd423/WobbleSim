...
class TelFitModel(TheoryModel):
            ...
            if value.unit.physical_type in unit.physical_type:
                if self.epoches:
                    if value.shape[0] == self.epoches:
                        ...
                        logging.warn(
                            "input not the correct size {}".format(self.epoches)
                        )
                else:
    ...
    def generate_transmission(self, star, detector, obs_times):
        ...
        for i, time in enumerate(obs_times):
            ...
            angle = np.arccos(1.0 / secz[i]) * 180 * u.deg / np.pi
            print(
            ...
            model = modeler.MakeModel(
                ...
                lowfreq=float(
                    1.0
                    / (
                        detector.lambmax.to(u.cm).value
                        + self.wave_padding.to(u.cm).value
                    )
                ),
                highfreq=float(
                    1.0
                    / (
                        detector.lambmin.to(u.cm).value
                        - self.wave_padding.to(u.cm).value
                    )
                ),
