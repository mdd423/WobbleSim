...
def dict_from_h5(hf, data):
    ...
    for key in hf.keys():
        if key == "obs_times":
            ...
            data[key] = coord.EarthLocation.from_geocentric(
                hf["loc"]["value"][()][0] * u.Unit(hf["loc"]["unit"][()][0]),
                hf["loc"]["value"][()][1] * u.Unit(hf["loc"]["unit"][()][0]),
                hf["loc"]["value"][()][2] * u.Unit(hf["loc"]["unit"][()][0]),
            )
        elif key == "target":
            data[key] = coord.SkyCoord(
                hf["target"]["ra"][0] * u.deg, hf["target"]["dec"][0] * u.deg
            )
        elif key == "value":
...
def save_dict_as_h5(hf, data):
    ...
    for key in data.keys():
        ...
        def save_time():
            ...
            times = np.array(
                [x.to_value("isot", subfmt="date_hms") for x in data[key]], dtype=dt
            )
...
class DetectorData:
    ...
    def plot_theory(self, ax, i, xy="", units=u.Angstrom, normalize=None, nargs=[]):
        self.plot_flux(
            ax,
            i,
            ["theory", "interpolated", "total", "flux"],
            ["theory", "interpolated", "total", "wave"],
            xy=xy,
            pargs={"color": "gray", **interpolated_settings},
            units=units,
            normalize=normalize,
            nargs=nargs,
        )
    ...
    def plot_lsf(self, ax, i, xy="", units=u.Angstrom, normalize=None, nargs=[]):
        self.plot_flux(
            ...
            ["theory", "interpolated", "total", "wave"],
            xy=xy,
            pargs={"color": "pink", **interpolated_settings},
            units=units,
            normalize=normalize,
            nargs=nargs,
        )
    ...
    def plot_star(self, ax, i, xy="", units=u.Angstrom, normalize=None, nargs=[]):
        self.plot_flux(
            ax,
            i,
            ["theory", "interpolated", "star", "flux"],
            ["theory", "interpolated", "total", "wave"],
            pargs={**star_settings, **interpolated_settings},
            xy=xy,
            units=units,
            normalize=normalize,
            nargs=nargs,
    ...
    def plot_gas(self, ax, i, xy="", units=u.Angstrom, normalize=None, nargs=[]):
        self.plot_flux(
            ax,
            i,
            ["theory", "interpolated", "gascell", "flux"],
            ["theory", "interpolated", "total", "wave"],
            xy=xy,
            units=units,
            pargs={**gas_settings, **interpolated_settings},
            normalize=normalize,
            nargs=nargs,
        )
        return ax
    ...
    def plot_tellurics(self, ax, i, xy="", units=u.Angstrom, normalize=None, nargs=[]):
        self.plot_flux(
            ax,
            i,
            ["theory", "interpolated", "tellurics", "flux"],
            ["theory", "interpolated", "total", "wave"],
            xy=xy,
            pargs={**tellurics_settings, **interpolated_settings},
            normalize=normalize,
            nargs=nargs,
        )
        return ax
    ...
    def plot_rvs(self, ax, units=u.km / u.s):
        now = at.Time.now()
        time = np.array(
            [(x - at.Time.now()).value for x in self["data"]["obs_times"]], dtype=float
        )
    ...
    def plot_rvs_minus_bcs(self, ax, units=u.km / u.s):
        ...
        ax.plot(
            self["data"]["times"] - now,
            (self["parameters"]["rvs"] - bcs).to(units).value,
            ".k",
        )
        return ax
    ...
    def plot_rvs_minus_bcs_mod_period(self, ax, units=u.km / u.s):
        ...
        ax.plot(
            (self["data"]["times"] - now) % self["parameters"]["period"],
            (self["parameters"]["rvs"] - bcs).to(units).value,
            ".k",
        )
        return ax
